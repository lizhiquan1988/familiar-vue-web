<template>
  <main>
    <div v-if="!loginFlag" class="text-container">
      <p>アカウント</p>
      <div>
        <div class="button_margin" @click="TransitToRegistration">登録</div>
        <div class="button_margin" @click="TransitToLogin">ログイン</div>
      </div>
    </div>
    <div v-else> 
...
export default {
  ...
  methods: {
    init() {
      ...
      } else if (localStorage.getItem('loginFlag') === "false") {
        this.loginFlag = false
      } else {
        this.loginFlag = true
      }
      const token = localStorage.getItem('token') === null? "": localStorage.getItem('token');
      FamiliarGlobal.userId = localStorage.getItem('userId') === null? "": localStorage.getItem('userId');
      FamiliarGlobal.LoginFlag = this.loginFlag
      FamiliarGlobal.token = token
      console.log("userId:"+FamiliarGlobal.userId+" loginflag:"+this.loginFlag + " token:"+FamiliarGlobal.token)
      // トークン認証を実施
      if(token!=="") {
    ...
    verifyToken(token){
      FamiliarService.verifyToken(token).then(response => {
    ...
    getLocationFromDB() {
      ...
      FamiliarService.findByUserId(FamiliarGlobal.userId).then(response => {
